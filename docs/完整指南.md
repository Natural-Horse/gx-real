...
本机夹爪的闭合零位在掉电或 SDK 重启后会丢失：静止（闭合）位电机 readout
≈ -1.265，若直接按 X5_GRIPPER_OPEN_READOUT=5.13142 换算会得到 -0.022m，
SDK 启动检查（必须落在 -0.005~0.093m）会直接报 Emergency 拒绝启动。因此
每次上电后、启动臂节点前，必须先跑标定脚本，把闭合零位写入电机：

bash scripts/calibrate_gripper.sh

脚本会自动检查没有臂进程占用 can0、确保 can0 UP，然后用临时
gripper_open_readout=-3.0 绕过启动检查进入 calibrate_gripper。按提示操作：

1. 手动把夹爪完全闭合，按回车。
2. 手动把夹爪完全打开，按回车。
3. 记下打印的 fully-open readout（本机应为约 5.13）。

标定完成后保持通电，不要重启 SDK，立即启动臂节点：

bash scripts/run_vla_arm_real.sh

验证方法：启动后 topic /arm/state 中 gripper_pos 完全闭合应约 0、完全打开
应约 0.088m。若换臂或换 SDK，把标定打印的 fully-open readout 更新到
real-wbc/modules/spacemouse_arm_node.py 的 X5_GRIPPER_OPEN_READOUT。
