...
- USB-CAN 转接器，推荐用 `bash scripts/find_can_dev.sh` 自动找 by-id 路径
  （优先 Openlight CANable2、其次 ARX）。
- Go2 手柄。
...
MCF 查询、释放和启动确认:
  unitree_sdk2/example/low_level/disable_sports_mode_go2.cpp
  scripts/disable_sports_mode_go2.sh
  scripts/run_leg12_real.sh
  real-wbc/modules/runtime_safety.py
...
前置脚本中的 MCF 工具必须输出 MCF release verified。如果当前 motion mode 非空，确认 Go2 已安全放置后按 Enter。
...
- 配置 can0。
- 检查 Go2 ROS2 topic。
- 通过 CheckMode()、ReleaseMode() 和再次 CheckMode() 释放并确认 Go2 MCF。

7.1 MCF 释放流程

释放 Unitree 内置运动控制框架的当前 motion mode，不再使用旧的：
RobotStateClient::ServiceSwitch("sport_mode", 0)
当前工具使用：
MotionSwitcherClient::CheckMode()
MotionSwitcherClient::ReleaseMode()
MotionSwitcherClient::CheckMode()
执行逻辑如下：
1. 第一次 CheckMode() 查询当前 robot form 和 motion mode。
2. 如果 motion mode 已经为空，直接输出 MCF release verified 并成功返回。
3. 如果仍有活动模式，工具会显示警告并等待操作者按 Enter。
4. ReleaseMode() 最多尝试 5 次，每次等待 1 秒。
5. 每次释放后再次调用 CheckMode()；只有 motion mode 为空才算成功。
6. 查询失败、释放失败或最终仍有活动模式时返回非零，WBC 不会启动。
prepare_real_run.sh 会在前置阶段调用一次释放工具；run_leg12_real.sh 在启动 Python 前还会重新执行并确认。第二次检查是幂等的：MCF 已释放时不会再次要求确认。
run_leg12_real.sh 只有在工具成功后才设置 GX_REAL_MCF_RELEASE_CONFIRMED=1。直接执行 real-wbc/scripts/run_wbc_leg12.py、或者释放工具返回失败时，低层节点不会启动。
MCF 释放后，真机上的 SportModeState 可能停止发布，这是正常现象。WBC 不再要求该 topic 持续存在；但如果之后重新收到新鲜的活动 motion mode 状态，会停止低层控制。不要再使用已经删除的 --allow-unknown-sport-mode 参数。
MCF 释放后不能使用依赖内置运动控制器的 unitree_auto、unitree_recoverystand 或 unitree_standup，真机主线使用 --standup-mode internal。执行释放前必须让 Go2 吊起或安全趴卧，因为释放动作会撤掉内置运动控制器。
...
不要用 SDK 自带 `python/examples/calibrate.py` 做关节自检：它使用默认
RobotConfig、没有软件零位偏移，本机夹爪零位掉电丢失时会报 `-0.022` 启动检查
失败（臂节点启动时会自动套偏移）。`calibrate.py` 仅用于换臂后重测夹爪标定
常量（见 `scripts/calibrate_gripper.sh`）。
...
查看当前可能占用 X5 的进程：
pgrep -af "run_spacemouse_arm|run_vla_arm|check_x5_joints|test_joint_control|arx5|candump"

8.3 夹爪标定（软件零位偏移，无需每次上电标定）

本机夹爪的闭合零位在掉电或 SDK 重启后会丢失：未标定时闭合位电机 readout
≈ -6.35、静止（全开）位 readout ≈ -1.27。SDK 已支持
`gripper_zero_offset`（RobotConfig 字段），节点启动时自动写入闭合位偏移，
读数和命令两条路径都套用同一偏移，因此【日常启动不需要交互式标定】：

- 常量定义：real-wbc/modules/arx5_gripper_calib.py
  - X5_GRIPPER_ZERO_OFFSET = -6.348（闭合位原始 readout）
  - X5_GRIPPER_OPEN_READOUT = 5.0784（闭合到全开的 readout 跨度）
- 换算：pos(m) = (angle - zero_offset) / open_readout * width；
  命令反向加回 zero_offset。
- 启动后 /arm/state 中 gripper_pos 完全闭合应约 0、完全打开应约 0.088m。

只有在更换机械臂或夹爪电机后才需要重新实测。重测方法：

bash scripts/calibrate_gripper.sh

脚本自动检查无臂进程占用 can0、确保 can0 UP，然后绕过 SDK 启动检查进入
calibrate_gripper：手动完全闭合按回车、完全打开按回车，记下打印的
fully-open readout，连同闭合位 readout 一起更新到 arx5_gripper_calib.py。
...
15.8 MotionSwitcherClient::CheckMode failed
说明 MCF 工具没有成功查询机器人。按顺序检查：
1. GX_REAL_NETWORK_IFACE 是否是连接 Go2 的实际网卡。
2. 网卡是否位于 192.168.123.xxx 网段。
3. Go2 是否上电并完成启动。
4. 是否有其他 Unitree SDK 进程占用或干扰通信。
5. 重新运行 scripts/disable_sports_mode_go2.sh eth0，让脚本检查并重建当前工具。

15.9 ReleaseMode failed 或 MCF is still active

先确认机器人已经吊起或安全趴卧，再停止 App、遥控器或其他可能重新启用内置 motion mode 的进程，然后重新运行：
scripts/disable_sports_mode_go2.sh eth0
工具只有在第二次 CheckMode() 返回空 motion mode 后才成功。不要跳过错误继续启动 WBC。

15.10 MCF release is not confirmed

说明直接运行了 real-wbc/scripts/run_wbc_leg12.py，或者启动脚本没有成功完成释放。不要手工设置环境变量，改用：
scripts/run_leg12_real.sh <原有参数>

15.11 MCF 释放后收不到 SportModeState
这是正常现象，不需要使用 --allow-unknown-sport-mode。如果之后重新收到活动 motion mode，说明内置控制被重新启用，应停止并重新执行完整启动流程。
...
- MCF 工具输出 MCF release verified。
- calibrate.py X5 can0 正常。
- SpaceMouse Arm 节点正常启动。
- WBC 看到 /arm/state.

开始运动：
- run_leg12_real.sh 再次确认 MCF 已释放。
