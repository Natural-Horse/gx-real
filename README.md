...
工具会用 `CheckMode()` 检查当前 motion mode，调用 `ReleaseMode()` 后再次验证；任何 SDK 错误或仍有活动模式都会返回失败。不要绕过这一步，因为 Go2 原厂高层控制和低层 `lowcmd` 会抢控制权。
...
- `Gripper position error: got -0.08x but should be in 0~0.088`：这是 X5 夹爪电机
  安装方向/零位与 `gripper_open_readout` 不匹配导致的校准问题。当前
  `run_spacemouse_arm.sh --model X5` 固定写入 `gripper_width=0.088`、
  `gripper_open_readout=5.0784`、`gripper_zero_offset=-6.348`（见
  `real-wbc/modules/arx5_gripper_calib.py`；本机闭合零位掉电易失，用软件偏移
  自动映射）。换臂/换电机后用 SDK `calibrate_gripper` 重新校准并更新该文件。
  启动日志必须出现 `Using persistent X5 gripper calibration`。详见
...
- MCF 释放后真机不再发布 `SportModeState` 属于正常现象；如果它重新出现并报告活动模式，WBC 会停止低层控制。
